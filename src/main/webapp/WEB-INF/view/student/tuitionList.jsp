                    ...
                    <tbody>
                        <c:forEach var="tuition" items="${tuitionList}">
                            <tr>
                                ...
                                <td>${tuition.tuiAmount}</td>
                                <td>${tuition.schAmount}</td>
                                <td>납입금 자리 -> 장학금이랑 해서 계산 방법 생각해봐야 할 듯</td>
								<c:if test="${tuition.status eq '처리중'}">
	                                <td>${tuition.status} tuition.status는 boolean타입, false면 납부x 납부 x면 버튼나오게, 납부 o면 납부 완료나오게 할까싶음</td>
								    	<a href="/student/tuition/${leaveOfAbsence.id}">
								        	<button class="btn btn-primary" id="tuitionButton" type="button">등록금 납부</button>
								    	</a>
								</c:if>
                            </tr>
                        </c:forEach>
