...
public class StuSubController {
	...
	public String readSubjectList(Model model, @PathVariable(name = "page") Integer page) {
		...
		List<SubjectDto> subjectListLimit = subjectList.subList(startIndex,
				Math.min(startIndex + pageSize, subjectCount));
		...
		// 중복을 제거한 강의 이름 리스트
		List<String> subNameList = subjectList.stream().map(SubjectDto::getName).distinct()
	...
	public String deletePreStuSubAppProc(@PathVariable(name = "subjectId") Integer subjectId,
	...
	public String insertStuSubAppProc(@PathVariable(name = "subjectId") Integer subjectId,
			@RequestParam(name = "type") Integer type) {
		// 수강 신청 기간이 아니라면 예외 처리
		if (SUGANG_PERIOD != 1) {
			throw new CustomRestfullException("수강 신청 기간이 아닙니다.", HttpStatus.BAD_REQUEST);
	...
	public String deleteStuSubAppProc(@PathVariable(name = "subjectId") Integer subjectId,
			@RequestParam(name = "type") Integer type) {
		// 수강 신청 기간이 아니라면 예외 처리
		if (SUGANG_PERIOD != 1) {
			throw new CustomRestfullException("수강 신청 기간이 아닙니다.", HttpStatus.BAD_REQUEST);
