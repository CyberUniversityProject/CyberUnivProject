...
<body>
	...
	<div class="container mb-5">
		<div class="row">
			...
			<div class="col-md-10">
				<h1 class="mt-5 mb-4">예비 수강신청 목록</h1>
				<hr />
					...
					수강 신청
						...
						<div class="d-flex justify-content-between align-items-start"
							...
							<div>
								<c:choose>
									<c:when
										test="${stuSubList.isEmpty() == false || stuSubListC.isEmpty() == false}">
										<c:if test="${stuSubList.isEmpty() == false}">
											<h4>
												<span style="font-weight: 600;"> <c:choose>
														<c:when test="${type == 0}">
											신청 내역&nbsp;
											<span style="color: gray; font-size: 18px;">[총
																${sumGrades}학점]</span>
														</c:when>
														<c:otherwise>
											신청 미완료 강의 목록
												...
												<tbody>
													...
													${stuSub.subDay} ${stuSub.startTime}:00-${stuSub.endTime}:00&nbsp;(${stuSub.roomId})							
															...
															<td class="sub--list--button--row"><c:choose>
																	<c:when test="${type == 0}">
																		<form action="/sugang/pre/${stuSub.subjectId}?type=1"
																			method="post">
																			<input type="hidden" name="_method" value="delete">
																			<button type="submit"
																				onclick="return confirm('예비수강신청을 취소하시겠습니까?');"
																				style="background-color: #a7a7a7;">취소</button>
																		</form>
																	</c:when>
																	<c:otherwise>
																		<form
																			action="/sugang/insertApp/${stuSub.subjectId}?type=1"
																			method="post">
																			<button type="submit"
												...
												<tbody>
													...
													${stuSub.subDay} ${stuSub.startTime}:00-${stuSub.endTime}:00&nbsp;(${stuSub.roomId})							
															...
															<td class="sub--list--button--row">
																<form
																	action="/sugang/deleteApp/${stuSub.subjectId}?type=1"
																	method="post">
																	<input type="hidden" name="_method" value="delete">
																	<button type="submit"
							...
							<c:choose>
								<%-- 예비 수강 신청 --%>
								<c:when test="${type == 0}">
									<a href="/sugang/pre/1"><button
											class="preStuSubList--button">강의 검색</button></a>
								</c:when>
								<%-- 수강 신청 --%>
								<c:otherwise>
									<a href="/sugang/application/1"><button
											class="preStuSubList--button">강의 검색</button></a>
								</c:otherwise>
							</c:choose>
						</div>
